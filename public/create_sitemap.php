...
// TODO: add in changefreq options

// Please edit these values before running your script.

//////////////////////////////////// Options ////////////////////////////////////

// $url = "http://www.pendletonunitedmethodist.org/"; // The Url of the site - the last '/' is needed
$url = url();
...
$sitemap_file = 'sitemap.xml'; // The file to which the result is written to - must be writable. The file name is relative from root.

// Stuff to be ignored...

// Ignore the file/folder if these words appear in the name
$always_ignore = array(
  'images','googleba345a928584cf66.html',
);

// These files will not be linked in the sitemap.
$ignore_files = array(
  'create_sitemap.php','BingSiteAuth.xml','404.php','error.php','configuration.php','include.inc'
);

// The script will not enter these folders
$ignore_folders = array(
	'cgi-bin','php_uploads','images','includes','lib','js','css','styles','stylesheets','system','stats','CVS','.svn'
);
...
if($_POST['starting_priority'] != "") {
  $starting_priority = filter_var($_POST['starting_priority'], FILTER_VALIDATE_INT, array("options"=>array("min_range"=>10,"max_range"=>100)));
} else {
...
';
	...
	foreach ($all_pages as $link) {
		...
		if(strpos($link,'tutorial')) $priority += 10;
...
function getFiles($cd) {
	...
	foreach($folders as $dir) {
		...
		if(count($more_pages)) $links = array_merge($links,$more_pages); // We need all thing in 1 single dimensional array.
	}
...
<form action="create_sitemap.php" method="post">
  Starting Priority : <input type="number" min="10" max="100" step="10" value="<?php echo $starting_priority; ?>" name="starting_priority" id="starting_priority" required>
